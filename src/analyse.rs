...
/// Prédiction de changement : compare deux images en mémoire, retourne le
/// bbox du changement + le % de pixels modifiés (predictive coding — le
/// cerveau ne traite que l'erreur de prédiction).
/// Retourne None si immobile (pas de changement significatif).
pub fn diff_bbox(a: &[u8], b: &[u8], seuil_px: i64) -> Result<Option<(u32, u32, u32, u32, f64)>, String> {
...
/// Crop + zoom d'une zone (en mémoire) — équivalent de crop_zoom_png mais
/// accepte des bytes PNG bruts (utilisé par --veille pour analyser la zone
/// CHANGÉE sans écrire de fichier).
pub fn crop_bytes_png(
    png_bytes: &[u8],
    x0: u32, y0: u32, x1: u32, y1: u32,
    facteur: u32,
) -> Result<Vec<u8>, String> {
    let img = image::load_from_memory(png_bytes).map_err(|e| format!("image: {e}"))?;
    crop_zoom_png(&img, x0, y0, x1, y1, facteur)
}
